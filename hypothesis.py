...
#    detailed_labels=False):
    ...
    def src_time_constraint(self,source,target):
        ...
        Args:
          source (str): full source name in gss_timestamp format
          target (str): full target name in gss_timestamp format

        Returns:
          bool: Truth value of src acceptance

        """
        _, source_gss_full, source_timestamp, _ \
            = re.split(r'(.*)_(\d+)', source)
        _, target_gss_full, target_timestamp, _ \
    ...
    def deQuantizer(self,
        ...
        if timestamp_list is None:
            if time_start is None and self.time_end is None:
                ...
                for gss_key in self.NMAP:
                    _, gss_full, time, _ \
                        = re.split(r'(.*)_(\d+)', gss_key)
                    time = int(time)
                    if (time <= time_end) and (gss_prefix in gss_key):
                        vals.append(
                            self.deQuantize_lowlevel(letter, self.NMAP[gss_key]))
            elif time_end is None:
                time_start = int(time_start)
                for gss_key in self.NMAP:
                    _, gss_full, time, _ = re.split(r'(.*)_(\d+)', gss_key)
                    time = int(time)
                    if (time_start <= time) and (gss_prefix in gss_key):
                        vals.append(
                            self.deQuantize_lowlevel(letter, self.NMAP[gss_key]))
            else: # both present
                time_start = int(time_start)
            ...
            for gss_key in self.NMAP:
                ...
                if time in timestamp_list:
                    vals.append(self.deQuantize_lowlevel(letter, self.NMAP[gss_key]))
